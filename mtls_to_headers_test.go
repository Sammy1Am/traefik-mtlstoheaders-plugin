...
func TestPassTLSClientCert_PEM(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			//TODO assert.Equal(t, http.StatusOK, res.Code, "Http Status should be OK")
			//TODO assert.Equal(t, "bar", res.Body.String(), "Should be the expected body")

			if test.expectedHeader != "" {
				//TODO expected := getCleanCertContents(test.certContents)
				//TODO assert.Equal(t, expected, req.Header.Get(xForwardedTLSClientCert), "The request header should contain the cleaned certificate")
			} else {
				//TODO assert.Empty(t, req.Header.Get(xForwardedTLSClientCert))
			}

			//TODO assert.Empty(t, res.Header().Get(xForwardedTLSClientCert), "The response header should be always empty")
		})
	}
}
...
WqeUSNGYV//RunTeuRDAf5OxehERb1srzBXhRZ3cZdzXbgR/`,
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			//TODO content := sanitize(test.toSanitize)

			//TODO expected := strings.ReplaceAll(test.expected, "\n", "")
			//TODO assert.Equal(t, expected, content, "The sanitized certificates should be equal")
		})
	}
}
